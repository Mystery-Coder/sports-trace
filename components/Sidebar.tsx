...
const modules = [
	{
		...
		children: [
			...
			{ href: "/screenshot", label: "Screenshot Decoder", icon: Camera },
...
export default function Sidebar() {
	...
	return (
		<aside
			className="glass-card-static flex flex-col w-65 min-h-screen p-4 gap-2"
			style={{
				borderRadius: 0,
				borderLeft: "none",
				borderTop: "none",
		...
		>
			...
			<Link href="/" className="flex items-center gap-3 px-3 py-4 mb-2">
				<div
					...
					style={{
						background:
							"linear-gradient(135deg, var(--st-cyan), var(--st-violet))",
					}}
				>
			...
			<div
				...
				style={{ background: "var(--st-border-subtle)" }}
			/>
			...
			<nav className="flex flex-col gap-1">
				{modules.slice(1).map((mod) => {
					const Icon = mod.icon;
					return (
						<div key={mod.id} className="sidebar-link disabled">
							<Icon size={18} />
